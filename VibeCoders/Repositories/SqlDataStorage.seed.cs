...
public partial class SqlDataStorage
{
    public void SeedPrebuiltWorkouts()
    {
        ...
        this.SeedTemplate(conn, "HIIT Fat Burner", new[]
        {
        ...
        this.SeedTemplate(conn, "Full Body Mass", new[]
        {
            ("Back Squat", "LEGS", 4, 8),
        ...
        this.SeedTemplate(conn, "Full Body Power", new[]
        {
            ("Deadlift", "BACK", 4, 5),
        ...
        this.SeedTemplate(conn, "Endurance Circuit", new[]
        {
            ("Push-Ups", "CHEST", 3, 20),
    ...
    public void SeedTestData()
    {
        ...
        int log1 = this.CreateLog(conn, clientId, DateTime.Now, "01:15:00", 450);
        this.AddSet(conn, log1, "Barbell Squat", 1, 10, 100.0);
        this.AddSet(conn, log1, "Barbell Squat", 2, 8, 105.0);
        this.AddSet(conn, log1, "Barbell Squat", 3, 6, 110.0);
        this.AddSet(conn, log1, "Romanian Deadlift", 1, 12, 80.0);
        this.AddSet(conn, log1, "Romanian Deadlift", 2, 12, 80.0);
        this.AddSet(conn, log1, "Romanian Deadlift", 3, 10, 85.0);
        this.AddSet(conn, log1, "Romanian Deadlift", 4, 8, 90.0);
        this.AddSet(conn, log1, "Calf Raises", 1, 15, 60.0);
        this.AddSet(conn, log1, "Calf Raises", 2, 15, 60.0);

        int log2 = this.CreateLog(conn, clientId, DateTime.Now.AddDays(-3), "00:55:00", 320);
        this.AddSet(conn, log2, "Bench Press", 1, 10, 80.0);
        this.AddSet(conn, log2, "Bench Press", 2, 8, 85.0);
        this.AddSet(conn, log2, "Bench Press", 3, 8, 85.0);
        this.AddSet(conn, log2, "Overhead Press", 1, 10, 40.0);
        this.AddSet(conn, log2, "Overhead Press", 2, 10, 40.0);

        int log3 = this.CreateLog(conn, clientId, DateTime.Now.AddDays(-7), "01:05:00", 400);
        this.AddSet(conn, log3, "Pull-ups", 1, 12, 0.0);
        this.AddSet(conn, log3, "Pull-ups", 2, 10, 0.0);
        this.AddSet(conn, log3, "Pull-ups", 3, 8, 0.0);
        this.AddSet(conn, log3, "Barbell Row", 1, 10, 60.0);
        this.AddSet(conn, log3, "Barbell Row", 2, 10, 60.0);
        this.AddSet(conn, log3, "Barbell Row", 3, 8, 65.0);
    }
    ...
    private int CreateLog(SqliteConnection conn, int clientId, DateTime date, string duration, int cals)
    {
        using var cmd = new SqliteCommand(@"
    ...
    private void AddSet(SqliteConnection conn, int logId, string exName, int setIndex, int reps, double weight)
    {
        using var cmd = new SqliteCommand(@"
...
public partial class SqlDataStorage
{
    ...
    public void SeedWorkoutMilestoneAchievements()
    {
        ...
        foreach (var milestone in TotalWorkoutsMilestoneEvaluator.DefaultMilestones)
        {
            ...
            using (var insertCmd = new SqliteCommand(insertSql, conn))
            {
                // CS1061 FIX: Properties updated to match the lowercase naming in the record definition
                insertCmd.Parameters.AddWithValue("@Title", milestone.title);
                insertCmd.Parameters.AddWithValue("@Description", milestone.description);
                insertCmd.Parameters.AddWithValue("@Threshold", milestone.threshold);
                insertCmd.ExecuteNonQuery();
